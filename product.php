...
<div class="card-container">
    ...
    if (isset($_POST['submit'])) {
        ...
        if (move_uploaded_file($_FILES["productImage"]["tmp_name"], $targetFile)) {
            ...
            $sql = "INSERT INTO product (category, description, image, price) VALUES ('$productName', '$productDescription', '$productImage', '$productPrice')";
...
while ($row = mysqli_fetch_assoc($result)) {
  echo '<div class="col-xl-4 col-md-3 col-sm-6 mt-5">';
 //echo '<div class="card" >';
 echo '<img class="card-img-top" src="' . $row['image'] . '" alt="' . $row['category'] . '" width="50%" height="330px">';
...
if (isset($_POST['update_product'])) {
    ...
    echo '</div>';
    echo '<div class="form-group">';
    echo '<label for="productImage">Product Image:</label><br>';
    echo '<input type="file" id="productImage" name="productImage"><br>';
    echo '<img src="' . $product['image'] . '" alt="Product Image" style="max-width: 200px;"><br>'; // Display current image
    echo '</div>';
    echo '<div class="form-group">';
...
//   $productImage = $_POST['productImage'];
  ...
  if ($_FILES['productImage']['name'] != '') {
    ...
    $productImage = $_FILES['productImage']['name'];
    $targetDirectory = "images/"; // Directory where you want to save the uploaded files
    $targetFile = $targetDirectory . basename($_FILES["productImage"]["name"]);

    if (move_uploaded_file($_FILES["productImage"]["tmp_name"], $targetFile)) {
        // File uploaded successfully, proceed with database update
        $sql = "UPDATE product SET category='$productName', description='$productDescription', image='$productImage', price='$productPrice' WHERE id=$product_id";
        if (mysqli_query($con, $sql)) {
    ...
    // No new file uploaded, update database with existing image path
    $sql = "UPDATE product SET category='$productName', description='$productDescription', price='$productPrice' WHERE id=$product_id";
    if (mysqli_query($con, $sql)) {
        echo "<p>Product updated successfully.</p>";
    } else {
        echo "Error updating product: " . mysqli_error($con);
