

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="css/Dcart.css">    
    <title>Order Confirmation</title>
    <link rel="icon" type="image/png" href="data:image/png;base64," sizes="64x64">
 
</head>
<body>
   <p>Order Placed Successfully</p>
  <!-- <img src="https://media.istockphoto.com/id/1385911418/photo/green-check-mark-approval-sign-on-pink-background.jpg?s=612x612&w=0&k=20&c=teBuP9QXcrNA0tfw1dLZMXd8kvYYy3Q_gOl8Ze6LWLs="> -->
   <p id="Thank">Thank you for your purchase !</p>
  <!-- <p id="id"> Your Order ID is : 1728392238</p> -->
  <p id="ordetail">You will receive an Order confirmation with details of your Order on your Registered e-mail ID</p>
  <a href="home.php" target="_blank">
    
  <button href="home.php" id="continue">Continue Shopping</button>
</a>
<?php

// Fetch order IDs
$sql1 = "SELECT order_id FROM `order`";
$result1 = mysqli_query($con, $sql1);

// Check status for each order
while ($row1 = mysqli_fetch_assoc($result1)) {
    $orderId = $row1['order_id'];
    
    // Fetch status for the current order
    $sql = "SELECT status FROM `order` WHERE order_id=$orderId";
    $result = mysqli_query($con, $sql);
    $row = mysqli_fetch_assoc($result);
    $status = $row['status'];

    // Print status message for the current order
    if ($status == 'Confirmed') {
        echo "<h1>Order  is confirmed</h1>";
    } else {
        echo "<h1>Order  is waiting for confirmation...</h1>";
    }
}
?>
</body>
</html>

